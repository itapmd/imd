...
void write_distrib(int steps)
{
  ...
  if (0==myid) {
    ...
    if (use_header) {
      write_dist_header(outpot, 0);
      write_dist_header(outkin, 1);
      fprintf(outminmax, "# contents count min_Epot max_Epot min_Ekin max_Ekin\n");
    }
...
void write_dist_header(FILE *out, int type) {
  ...
  fprintf(out, "%d %d ", dist_dim.x, dist_dim.y);
#ifndef TWOD
  fprintf(out, "%d", dist_dim.z);
#endif
  fprintf(out, "\n");
