...
void write_distrib_header(FILE *out, char *type)
{
  ...
  if (dist_binary_io)
    ...
    c = 'A';
  if (dist_has_coords)
    fprintf(out, "# format d %c %d 1\n", c, DIM);
  else
    fprintf(out, "# format d %c 0 1\n",  c);

  /* contents line */
  if (dist_has_coords)
#ifdef TWO
    fprintf(out, "# contents x y %s\n",   type);
#else
    fprintf(out, "# contents x y z %s\n", type);
#endif
  else
    fprintf(out, "# contents %s\n",       type);
...
void write_press_dist_header(FILE *out)
{
  ...
  */
    c = 'A';
  if (dist_has_coords)
    fprintf(out, "# format d %c %d %d\n", c, DIM, (DIM*(DIM+1))/2);
  else
    fprintf(out, "# format d %c 0 %d\n",  c,      (DIM*(DIM+1))/2);

  /* contents line */
#ifdef TWOD
  if (dist_has_coords)
    fprintf(out, "# contents x y P_xx P_yy P_xy density Ekin Epot\n");
  else
    fprintf(out, "# contents P_xx P_yy P_xy density Ekin Epot\n");
#else
  if (dist_has_coords)
    fprintf(out, 
      "# contents x y z P_xx P_yy P_zz P_yz P_zy P_xy density Ekin Epot\n");
  else
    fprintf(out, 
      "# contents P_xx P_yy P_zz P_yz P_zx P_xy density Ekin Epot\n");
#endif
...
void write_press_dist_shock_header(FILE *out)
{
  ...
  */
    c = 'A';
  if (dist_has_coords)
    fprintf(out, "# format d %c 1 %d\n", c, 2*DIM+3);
  else
    fprintf(out, "# format d %c 0 %d\n", c, 2*DIM+3);

  /* contents line */
#ifdef TWOD
  if (dist_has_coords) fprintf(out, "# contents x P_xx P_yy density");
  else                 fprintf(out, "# contents P_xx P_yy density");
  fprintf(out, " Ekin_xx Ekin_xxu Ekin_yy Epot\n");
#else
  if (dist_has_coords) fprintf(out, "# contents x P_xx P_yy P_zz density");
  else                 fprintf(out, "# contents P_xx P_yy P_zz density");
