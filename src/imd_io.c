...
void write_header_ef(FILE *out)
{
  ...
  /* binary_io for checkpoints not implemented
    ...
    */
    c = 'A';
  fprintf(out, "#1 format a %c 1 1 1 %d %d 1\n", c, DIM, DIM);
  
  /* contents line */
#ifdef TWOD
  fprintf(out, "#2 contents number type mass x y vx vy Epot\n");
#else
  fprintf(out, "#2 contents number type mass x y z vx vy vz Epot\n");
#endif
  ...
  fprintf(out, "#5 box_z \t%.16e %.16e %.16e\n", box_z.x , box_z.y , box_z.z);
#endif

  /* endheader line */
  fprintf(out, "#6 Generated by %s on %s", progname, ctime(&now));
  fprintf(out, "#7 endheader\n");
...
void write_header_sqd(FILE *out)
{
  ...
  /* binary_io for checkpoints not implemented
    ...
    */
  c = 'A';
  fprintf(out, "#1 format a %c 0 1 0 0 0 %d\n", c, DIM);
  
  /* contents line */
#ifdef TWOD
  fprintf(out, "#2 contents type d^2_x d^2_y\n");
#else
  fprintf(out, "#2 contents type d^2_x d^2_y d^2_z\n");
#endif

  /* endheader line */
  time(&now);
  fprintf(out, "#3 Generated by %s on %s", progname, ctime(&now));
  fprintf(out, "#4 endheader\n");
}
