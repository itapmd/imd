...
void vvSoftImg::copy(AlignType align, vvSoftImg* src)
{
  ...
  if (width > src->width || height > src->height)    // if destination image is larger than source:
    memset(data, 0, PIXEL_SIZE * width * height);    // clear destination image
...
*/
void vvSoftImg::warp(vvMatrix4* w, vvSoftImg* srcImg)
{
  vvMatrix4 inv;              // inverted warp matrix
  vvVector3 src;              // pixel in source image
...
/** Warp the current image to the OpenGL viewport using 2D texture mapping.
  @param w 4x4 warp matrix, only 2D components are used
*/
void vvSoftImg::warpTex(vvMatrix4* w)
{
#ifndef VV_REMOTE_RENDERING
  ...
  glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE);    // try GL_REPLACE and GL_MODULATE
...
*/
  ...
  else
  {
    ...
    glBegin(GL_QUADS);
      ...
      glTexCoord2f(0.0f, 0.0f); glVertex3f( 0.0f,                 0.0f,                   ZPOS);    // bottom left
      glTexCoord2f(1.0f, 0.0f); glVertex3f((float)imgUsed->width, 0.0f,                   ZPOS);    // bottom right
      glTexCoord2f(1.0f, 1.0f); glVertex3f((float)imgUsed->width, (float)imgUsed->height, ZPOS);    // top right
      glTexCoord2f(0.0f, 1.0f); glVertex3f( 0.0f,                 (float)imgUsed->height, ZPOS);    // top left
    glEnd();
  }
...
void vvSoftImg::saveGLState()
{
  ...
  // Store raster position:  
	glGetFloatv(GL_CURRENT_RASTER_POSITION, glsRasterPos); // memorize raster position
