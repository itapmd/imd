//****************************************************************************
// Filename:            vvsocket.cpp
// Project Affiliation: none
// Funcionality:        Provides system independent socket functionality
...
vvSocket::ErrorType vvSocket::init_server_tcp()
{
        ...
        if (sock_buffsize == 0)
        {
#if !defined(WIN32) && defined(VV_BDP)   
...
vvSocket::ErrorType vvSocket::connect_timeo()
{
        ...
        while (cr != 0)
        {
                ...
                if (cr != 0)
                {
                        ...
                        if (error!=WSAEALREADY && error!=WSAEWOULDBLOCK)   // ignore specific messages because they occurs repeatedly
...
*/
vvSocket::vvSocket(int portnumber, char* servername, SocketType st, int clminport, int clmaxport)
...
/** Signal function for timeouts
 ...
 @param func
*/
Sigfunc *vvSocket::Signal(int signo, Sigfunc *func)	/* for signal() function */
{
	Sigfunc	*sigfunc;
...
/** Writes to a socket with a timeout.Calls writen_tcp() for TCP Sockets
 ...
 @param size  number of bytes to write.
*/
vvSocket::ErrorType vvSocket::write_timeo(uchar* dataptr, size_t size)
{
        ssize_t  s;
...
/** Writes to a socket without a timeout. Calls writen_tcp() for TCP Sockets
 ...
 @param size  number of bytes to write.
*/
vvSocket::ErrorType vvSocket::write_nontimeo(uchar* dataptr, size_t size)
{
        ssize_t  s;
...
/** Function to write data to the socket.
 ...
 @param size  number of bytes to write.
*/
vvSocket::ErrorType vvSocket::write_data(uchar* dataptr, size_t size)
{
        ErrorType retval;
...
*/
vvSocket::ErrorType vvSocket::write_string(char* s)
{
        ErrorType ret;
        int len = strlen(s);
