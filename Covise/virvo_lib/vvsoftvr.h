...
/** A new rendering algorithm based on shear-warp factorization.
  ...
  @see vvSoftPer
*/
class vvSoftVR : public vvRenderer
{
  protected:
    ...
    vvMatrix4 owView;         ///< viewing transformation matrix from object space to world space
    vvMatrix4 osPerm;         ///< permutation matrix
    vvMatrix4 wvConv;         ///< conversion from world space to OpenGL viewport space
    vvMatrix4 oiShear;        ///< shear matrix from object space to intermediate image space
    vvMatrix4 ivWarp;         ///< warp object from intermediate image space to OpenGL viewport space
    vvMatrix4 iwWarp;         ///< warp object from intermediate image space to world space
    ...
    uchar preIntTable[PRE_INT_TABLE_SIZE][PRE_INT_TABLE_SIZE][4]; ///< size of pre-integrated LUT ([sf][sb][RGBA])
