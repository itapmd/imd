...
*/
  ...
  for (j = 0; j < number_of_pin_lists; j++)
  {
    ...
    a1 = (unsigned char)fgetc(dz);
    fseek(dz, octals2int(0, 0, 0, a1), SEEK_CUR);     // description text
    fseek(dz, int(17 * number_of_pins[j]), SEEK_CUR); // pin-entries
    if (vvDebugMsg::isActive(3))
      cerr << "						[ OK ]" << endl;
  }
...
int vvvffile::writeFile(void)
{
  ...
  fputc(46, dz);                                // .
  fputc((int(version_number / 10) % 10 + 48), dz);  // x
  ...
  for (i = 1; i < (temp_array_number + 1); i++)
  {
    ...
    while ((k - j) > 4096)
    {
      fread(iopuffer, 1, 4096, dt);
      fwrite(iopuffer, 1, 4096, dz);
      j += 4096;
    }

    fread(iopuffer, 1, (k - j), dt);
    fwrite(iopuffer, 1, (k - j), dz);
    if (vvDebugMsg::isActive(3))
      cerr << "			[ OK ]" << endl;
  }
...
int vvvffile::addToDataFile(int data_array_length, unsigned char coding, unsigned char* data)
{
  ...
  fwrite(data, 1, data_array_length, dt);
  if (vvDebugMsg::isActive(3))
    cerr << " 				[ OK ]" << endl;
                             ...
                             char*  channel_description)
{
  ...
  fread(r_channel, 1, 256, dz);
  fseek(dz, 7L, SEEK_CUR);
  fread(g_channel, 1, 256, dz);
  fseek(dz, 7L, SEEK_CUR);
  fread(b_channel, 1, 256, dz);
  fseek(dz, 7L, SEEK_CUR);
  fread(a_channel, 1, 256, dz);
...
int vvvffile::readHeaderNtBoxBox(void)
{
  ...
  {
    ++frames_number;
    fscanf(dz, "%d%f%f%f%f%f%f%f", &N, &t, &x1, &y1, &z1, &x2, &y2, &z2);
    for (i = 0; i < N; i++)
      fgets(str, 1023, dz);
  }
...
int vvvffile::readDataxyzvxvyvzrnc(unsigned char* data, int c)
{
  ...
  while (i != c)
  {
    ...
    (void)fscanf(dz, "%d%f%f%f%f%f%f%f", &N, &t, &x1, &y1, &z1, &x2, &y2, &z2);
    for (j = 0; j < (N + 1); j++)
      fgets(str, 1023, dz);
...
*/
    fread(data, 1, data_array_length, dz);
