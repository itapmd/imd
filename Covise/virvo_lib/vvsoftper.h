...
/** This class is the implementation of a new volume rendering algorithm 
  ...
  @see vvSoftImg
*/
class vvSoftPer : public vvSoftVR
{
  private:
    vvVector4 oEye;         ///< eye position [object space]
    vvVector4 sEye;         ///< eye position [standard object space]
    vvMatrix4 shift;        ///< shift volume if eye in slice 0 = T_shift
    vvMatrix4 scale;        ///< scale front voxel slice to 1:1 pixel:voxel ratio
    vvMatrix4 diConv;       ///< convert deformed space to intermediate image space
    vvMatrix4 sdShear;      ///< shear matrix from standard object space to deformed (sheared) space
