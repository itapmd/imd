...
//----------------------------------------------------------------------------
/// Constructor.
vvSoftPer::vvSoftPer(vvVolDesc* vd) : vvSoftVR(vd)
{
  vvDebugMsg::msg(1, "vvSoftPer::vvSoftPer()");

  rendererType = SOFTPER;

  setQuality(quality);    // not only sets quality but also resizes intermediate image
}
...
#define FLOAT_MATH        // undefine for integer math
  ...
  if (iPosX<0 || iPosX+iSlice[0]>=intImg->width ||    // return if slice doesn't fit in intermediate image
      iPosY<0 || iPosY+iSlice[1]>=intImg->height ||
      iSlice[0]<=0 || iSlice[1]<=0)
    return;

  iLineOffset = intImg->PIXEL_SIZE * (intImg->width - iSlice[0]);
  vScalar     = raw[principal] + vd->bpv * (slice * len[0] * len[1] + (len[1] - 1) * len[0]);
  vPosX       = 0;
  vPosY       = (len[1] - 1) << 16;
  vFracY      = 0;
  ...
  if (from != -1) // render only specific lines?
  {
    ...
    {
      vFracY  += vStepY;
      vScalar -= (vFracY >> 16) * len[0] * vd->bpv;
      vPosY   -= vStepY;
  ...
  for (iy=0; iy<iSlice[1]; ++iy)  
  {
    ...
    for (ix=0; ix<iSlice[0]; ++ix)
    {
      ...
      if ((ia<255) &&           // early ray termination for opaque image pixels
        (!clipMode || (!isVoxelClipped(vPosX >> 16, vPosY >> 16, slice))))     // skip clipped voxels
      {
...
void vvSoftPer::compositeSliceBilinear(int slice, int from, int to)
{
  ...
  vSliceBase  = raw[principal] + vd->bpv * slice * len[0] * len[1];
  ...
  for (iy=0; iy<iSlice[1]; ++iy) 
  {
    ...
    for (ix=0; ix<iSlice[0]; ++ix)
    {
      ...
      if ((ia<255) &&           // early ray termination for opaque image pixels
        (clipMode==false || (!isVoxelClipped((int)vPosX, (int)vPosY, slice))))     // skip clipped voxels
      {
        // Determine voxel color components and scale to [0..1]:
        if (zoomMode)
  ...
  float* r, float* g, float* b, float* a)
{
  ...
  if (left<0 || left+1>=len[0] || top<0 || top+1>=len[1])
  {
    ...
    vScalar[0] = sliceBase + vd->bpv * (top * len[0] + left);
  ...
  vScalar[1] = sliceBase + vd->bpv * (top * len[0] + left);
  vScalar[2] = vScalar[1] + vd->bpv;
  vScalar[0] = vScalar[1] + vd->bpv * len[0];
  vScalar[3] = vScalar[0] + vd->bpv;
  ...
  float* r, float* g, float* b, float* a)
{
  ...
  scalar = sliceBase + vd->bpv * (top * len[0] + left);
  lineOffset = vd->bpv * len[0];
  *r = *g = *b = *a = 0.0f;
  numVoxels = w * h;
  for (y=0; y<h; ++y)
  {
...
void vvSoftPer::setQuality(float q)
{
  ...
  q = 1.0f;    
#endif   
  vvRenderer::setQuality(q);

  intImgSize = (int)(SIZE_FACTOR * (2.0f * quality) * ts_max(vd->vox[0], vd->vox[1], vd->vox[2])); 
  if (intImgSize<1) 
  {
    intImgSize = 1;
    quality = 1.0f / (SIZE_FACTOR * 2.0f * ts_max(vd->vox[0], vd->vox[1], vd->vox[2]));
  }
...
{
  vvMatrix4 woView;   // inverse of viewing transformation matrix
...
void vvSoftPer::findPrincipalAxis()
{
  ...
  vvVector3 xbase, ybase, zbase;  // coordinate system base vectors in object space
...
void vvSoftPer::findScaleMatrix()
{
  ...
  scale.scale(2.0f * quality, 2.0f * quality, 1.0f);      
...
{
  vvMatrix4 ioShear;      // inverse of oiShear
