...
/* ---------------------------------------------------------
   ...
   <command line>  input Emin Emax x0 y0 b0 h0  b1 h1

   with input : name of input file (see below)
                (output = input.ppm)
        Emin Emax : bounderies of kinetic energy
   ...
   every record describes one atom with 

   x y Ekin Epot type

   with  x y       : atom coordinates
         Ekin Epot : kinetic and potential energy of atom
         size      : type of atom  
...
typedef       int integer;
  ...
  /*
  TopMatrix = calloc( col*row, sizeof( Triple ) );
  */
  ...
  do 
  {
    ...
    x    = picbuf.pos_x;
    y    = picbuf.pos_y;
    Ekin = picbuf.E_pot;   /* Compute color using potential energy */
    dE   = Emax-Emin;
